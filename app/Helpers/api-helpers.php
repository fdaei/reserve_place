...
function convertEnglishToPersianNumbers($string) {
...
if (!function_exists('getConfigs')) {
    function getConfigs($key,$default="")
    {
        static $configs = null;

        if ($configs === null) {
            $configs = Config::all()->keyBy('title');
        }

        return $configs->has($key) ? $configs->get($key)->value : $default;
    }
}
...
if (!function_exists("jalaliDate")) {
    ...
    function jalaliDate($format, $timestamp = '',  $time_zone = 'Asia/Tehran', $tr_num = 'en')
    {
...
function roundUp($number, $precision = 10) {
...
function getMonthNameListLastyearForChart(){
    $monthsInfo=[];
    $finishedLoop=true;
    $startedLoopOnOtherMonth=false;
    $thisMonth = jalaliDate("F", time());
    $i=0;
    while ($finishedLoop) {
        $monthTime=strtotime("-".$i." day");
        $monthName = jalaliDate("F", $monthTime);
        if ($startedLoopOnOtherMonth){
            if ($monthName==$thisMonth){
                $finishedLoop=false;
                continue;
            }
        }
        if ($monthName!=$thisMonth){
            $startedLoopOnOtherMonth=true;
        }
        if (!isset($monthsInfo[$monthName])){
            $sortedClients[$monthName]=[];
            $gregorianDate=jalaliDate("Y-m-",$monthTime).jalaliDate("t",$monthTime);
            $gregorianDateArray=explode("-",$gregorianDate);
            $monthsInfo[$monthName]["end"]=strtotime(jalali_to_gregorian($gregorianDateArray[0],$gregorianDateArray[1],$gregorianDateArray[2],"-")." 00:00:00");
            $gregorianDate=jalaliDate("Y-m-",$monthTime)."1";
            $gregorianDateArray=explode("-",$gregorianDate);
            $monthsInfo[$monthName]["start"]=strtotime(jalali_to_gregorian($gregorianDateArray[0],$gregorianDateArray[1],$gregorianDateArray[2],"-")." 23:59:59");
            $monthsInfo[$monthName]["created"]=0;
            $monthsInfo[$monthName]["cafebazaar"]=0;
            $monthsInfo[$monthName]["myket"]=0;
            $monthsInfo[$monthName]["application"]=0;
            $monthsInfo[$monthName]["website"]=0;
            $monthsInfo[$monthName]["price"]=0;
        }
        $i=$i+10;
    }
    return $monthsInfo;
}
function getJalaliDayList($fromDate = null, $toDate = null)
{
    $daysInfo = [];
    ...
    if (empty($fromDate) || empty($toDate)) {
        ...
        [$fromY, $fromM, $fromD] = explode('/', $fromDate);
        [$toY, $toM, $toD] = explode('/', $toDate);

        $start = strtotime(jalali_to_gregorian($fromY, $fromM, $fromD, '-') . " 00:00:00");
        $end = strtotime(jalali_to_gregorian($toY, $toM, $toD, '-') . " 23:59:59");
    }

    for ($time = $start; $time <= $end; $time += 86400) {
        $jalaliMonth = jalaliDate("F", $time);
        $jalaliDay = jalaliDate("d", $time);
        $key = $jalaliMonth . '-' . $jalaliDay;

        $gregorianDate = jalaliDate("Y-m-d", $time);
        $parts = explode("-", $gregorianDate);
        $timestamp = strtotime(jalali_to_gregorian($parts[0], $parts[1], $parts[2], "-") . " 00:00:00");
    ...
    return array_reverse($daysInfo);
}
function getJalaliWeekDayList()
{
    $weekDays = [
        'Saturday'   => 'شنبه',
        'Sunday'     => 'یکشنبه',
    ...
    for ($i = 6; $i >= 0; $i--) {
        ...
        $englishWeekDay = date("l", $time); // Saturday, Sunday, etc.
        $persianWeekDay = $weekDays[$englishWeekDay];

        $jalaliDate = jalaliDate("Y-m-d", $time);
        $parts = explode("-", $jalaliDate);
        $timestamp = strtotime(jalali_to_gregorian($parts[0], $parts[1], $parts[2], "-") . " 00:00:00");

        $daysInfo[$persianWeekDay] = [
            "timestamp" => $timestamp,
            "created"   => 0,
            "cafebazaar"=>0,
            "myket"=>0,
...
function getLast24HoursJalali()
{
    $hoursInfo = [];

    for ($i = 23; $i >= 0; $i--) {
        $time = strtotime("-{$i} hours");
        $hourLabel = date("H:00", $time); // مثل 13:00

        $jalaliDate = jalaliDate("Y-m-d", $time);
        $parts = explode("-", $jalaliDate);

        // تبدیل به تاریخ میلادی برای timestamp دقیق
        $gregorian = jalali_to_gregorian($parts[0], $parts[1], $parts[2], "-");
        $hour = date("H", $time);
        $timestamp = strtotime("$gregorian $hour:00:00");
    ...
    return array_reverse($hoursInfo);
}
